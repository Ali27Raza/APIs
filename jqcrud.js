    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        for (var i = 0; i < response.length; i++) {
          var rec = response[i];
          recipes.append(
            `<div class="recipe" data-id="${rec._id}"><h3>${rec.title}</h3><p><button type="button" class="btn btn-danger btn-sm float-right">delete</button><button type= "button" class="btn btn-warning btn-sm float-right">Edit</button> ${rec.body}</p></div>`
          );
